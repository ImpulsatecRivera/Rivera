...
const viajesClienteSchema = new Schema(
  {
    ...
    rutas: {
      ...
      default: [], // ✅ evita crashes si rutas viene vacío
    },
    ...
    pagos: {
      ...
      default: [], // ✅ evita undefined al hacer push
    },
...
viajesClienteSchema.pre("save", function (next) {
  ...
  (this.rutas || []).forEach((ruta) => {
    ...
    if (ruta.cantidadViajes && ruta.montoPorViaje) {
      ...
      ruta.montoTotal = ruta.montoTotal || 0;
    }
...
// ❌ QUITADO: viajesClienteSchema.index({ reporteId: 1 });
//    Porque "unique: true" en reporteId ya crea ese índice y causaba el warning.
...
viajesClienteSchema.statics.generarReporteId = async function () {
  const ultimoReporte = await this.findOne().sort({ reporteId: -1 }).select("reporteId");

  if (!ultimoReporte) return "REP001";

  const numero = parseInt(String(ultimoReporte.reporteId).replace("REP", ""), 10) + 1;
...
viajesClienteSchema.statics.obtenerReporteMensual = async function (mes, año) {
  ...
  const granTotal = reportes.reduce(
    (acc, reporte) => ({
      totalViajes: acc.totalViajes + (reporte.totalViajes || 0),
      totalMonto: acc.totalMonto + (reporte.montoTotalGeneral || 0),
