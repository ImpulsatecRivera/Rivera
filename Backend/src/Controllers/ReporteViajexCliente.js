...
// ✅ SOLO LO NECESARIO: helper para evitar crash si no existe montoTotal
const montoTotalRuta = (ruta) => {
  const cant = Number(ruta?.cantidadViajes || 0);
  const porViaje = Number(ruta?.montoPorViaje || 0);
  const mt = ruta?.montoTotal;
  if (mt !== undefined && mt !== null && !Number.isNaN(Number(mt))) return Number(mt);
  return cant * porViaje;
};
...
ReportesViajesRoutes.generarPDFEstiloTabla = async (req, res) => {
  ...
  try {
    ...
    // ✅ Buscar directamente los reportes
    ...
    reportes.forEach((reporte) => {
      ...
      // ✅ SOLO LO NECESARIO: totales seguros
      const totalViajesCliente =
        Number(reporte.totalViajes || 0) ||
        rutasActivas.reduce((sum, r) => sum + Number(r?.cantidadViajes || 0), 0);

      const totalMontoCliente =
        Number(reporte.montoTotalGeneral || 0) ||
        rutasActivas.reduce((sum, r) => sum + montoTotalRuta(r), 0);
      ...
      filasHTML += `
        <tr>
          ...
          <td class="cell-monto">$${montoTotalRuta(rutasActivas[0]).toFixed(2)}</td>
          <td rowspan="${rutasActivas.length}" class="cell-total">$ ${totalMontoCliente.toFixed(2)}</td>
        </tr>
      `;

      // Filas adicionales del cliente
      for (let i = 1; i < rutasActivas.length; i++) {
        filasHTML += `
          <tr>
            ...
            <td class="cell-monto">$${montoTotalRuta(rutasActivas[i]).toFixed(2)}</td>
          </tr>
        `;
      }
...
`;
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=viajes-${obtenerNombreMes(mesNum)}-${anoNum}.pdf`
    );
    res.send(pdfBuffer);
  } catch (error) {
...
ReportesViajesRoutes.generarPDFClienteIndividual = async (req, res) => {
  ...
  try {
    ...
    // ✅ SOLO LO NECESARIO: no revienta si rutas es undefined y no deja vacío si 'activa' no existe
    const rutasActivas = (cliente.rutas || []).filter((r) => r.activa !== false);

    // ✅ totales seguros
    const totalViajes =
      Number(cliente.totalViajes || 0) ||
      rutasActivas.reduce((s, r) => s + Number(r?.cantidadViajes || 0), 0);

    const montoTotalGeneral =
      Number(cliente.montoTotalGeneral || 0) ||
      rutasActivas.reduce((s, r) => s + montoTotalRuta(r), 0);
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .header { text-align:center; margin-bottom:40px; border-bottom:4px solid #2563eb; padding-bottom:25px; }
    .header h1 { color:#2563eb; font-size:32px; margin-bottom:8px; font-weight:700; text-transform:uppercase; }
    .header .subtitle { color:#64748b; font-size:16px; font-weight:500; }
    .id-badge { display:inline-block; background:#f1f5f9; padding:8px 16px; border-radius:8px; font-size:12px; color:#475569; margin-top:10px; font-family: monospace; }
    .section { margin-bottom:35px; background:#f8fafc; padding:25px; border-radius:10px; border-left:5px solid #2563eb; }
    .section-title { color:#2563eb; font-size:20px; font-weight:700; margin-bottom:20px; }
    ...
    .info-item label { display:block; font-weight:600; color:#64748b; font-size:11px; margin-bottom:8px; text-transform:uppercase; }
    .info-item .value { color:#1e293b; font-size:15px; font-weight:500; }
    table { width:100%; border-collapse:collapse; margin-top:20px; background:white; border-radius:8px; overflow:hidden; }
    thead { background: linear-gradient(135deg, #2563eb 0%, #1e40af 100%); color:white; }
    ...
    .total-section { margin-top:30px; background: linear-gradient(135deg, #1e40af 0%, #2563eb 100%); color:white; padding:25px; border-radius:10px; }
    .total-grid { display:grid; grid-template-columns: repeat(2, 1fr); gap:20px; }
    .total-item { text-align:center; }
    .total-item label { font-size:12px; opacity:0.9; margin-bottom:8px; }
    .total-item .value { font-size:24px; font-weight:700; }
    .footer { margin-top:40px; text-align:center; color:#64748b; font-size:11px; border-top:2px solid #e2e8f0; padding-top:20px; }
  </style>
</head>
<body>
  <div class="container">
    ...
    <div class="section">
      ...
      <div class="info-grid">
        ...
        <div class="info-item">
          <label>Período</label>
          <div class="value">${obtenerNombreMes(cliente.mes)} ${cliente.año}</div>
        </div>
        ${cliente.telefono ? `
        <div class="info-item">
        ...
        <tbody>
          ${rutasActivas.map((ruta, index) => `
            <tr>
              ...
              <td class="text-right">$${Number(ruta.montoPorViaje || 0).toFixed(2)}</td>
              <td class="text-right"><strong>$${montoTotalRuta(ruta).toFixed(2)}</strong></td>
            </tr>
          `).join("")}
        </tbody>
      </table>
...
`;
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=viajes-${cliente.clienteNombre}-${Date.now()}.pdf`
    );
    res.send(pdfBuffer);
  } catch (error) {
    if (browser) await browser.close();
    ...
    const mesesHTML = reportes
      .map((reporte, idx) => {
        ...
        reporte.clientes.forEach((cliente) => {
          // ✅ SOLO LO NECESARIO: no dejes vacío si activa no existe
          const rutasActivas = (cliente.rutas || []).filter((r) => r.activa !== false);
          if (rutasActivas.length === 0) return;

          const totalCliente =
            Number(cliente.montoTotal || 0) ||
            rutasActivas.reduce((s, r) => s + montoTotalRuta(r), 0);
          ...
          filasHTML += `
            <tr>
              ...
              <td class="cell-viajes">${rutasActivas[0].cantidadViajes}</td>
              <td class="cell-monto">$${montoTotalRuta(rutasActivas[0]).toFixed(2)}</td>
              <td rowspan="${rutasActivas.length}" class="cell-total">$ ${totalCliente.toFixed(2)}</td>
            </tr>
          `;
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .main-header { text-align:center; margin-bottom:40px; padding-bottom:20px; border-bottom:3px solid #000; }
    .main-header h1 { font-size:26px; font-weight:bold; margin-bottom:10px; }
    .mes-section { margin-bottom:50px; page-break-inside: avoid; }
    .mes-section h2 { text-align:center; background:#f0f0f0; padding:15px; border:2px solid #000; margin-bottom:20px; font-size:20px; }
    table { width:100%; border-collapse:collapse; border:2px solid #000; margin-bottom:20px; }
    ...
    .resumen-final { margin-top:40px; padding:25px; background:#f5f5f5; border:3px solid #000; text-align:center; }
    .resumen-final h3 { font-size:20px; margin-bottom:15px; }
    .resumen-final .total-final { font-size:28px; font-weight:bold; margin-top:10px; }
  </style>
</head>
<body>
...
`;
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=viajes-comparativo-${anoNum}.pdf`
    );
    res.send(pdfBuffer);
  } catch (error) {
    if (browser) await browser.close();
...
ReportesViajesRoutes.generarPDFConsolidadoAnual = async (req, res) => {
  ...
  try {
    ...
    // ✅ SOLO LO NECESARIO: tu modelo usa "año"
    ...
    // Estadísticas generales (seguros)
    const totalGeneral = clientes.reduce((sum, c) => sum + Number(c.montoTotalGeneral || 0), 0);
    const totalViajes = clientes.reduce((sum, c) => sum + Number(c.totalViajes || 0), 0);
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    body { font-family: Arial, sans-serif; padding: 30px; }
    .header { text-align:center; background: linear-gradient(135deg, #2563eb 0%, #1e40af 100%); color:white; padding:30px; border-radius:12px; margin-bottom:30px; }
    .header h1 { font-size:28px; margin-bottom:10px; }
    .stats-grid { display:grid; grid-template-columns: repeat(4, 1fr); gap:15px; margin-bottom:30px; }
    .stat-card { background:#f8fafc; padding:20px; border-radius:10px; text-align:center; border-left:4px solid #2563eb; }
    .stat-card label { font-size:11px; color:#64748b; text-transform:uppercase; margin-bottom:10px; display:block; }
    .stat-card .value { font-size:24px; font-weight:700; color:#2563eb; }
    table { width:100%; border-collapse:collapse; font-size:11px; }
    thead { background: linear-gradient(135deg, #2563eb 0%, #1e40af 100%); color:white; }
    th { padding:10px 8px; text-align:left; font-weight:600; text-transform:uppercase; }
    td { padding:10px 8px; border-bottom:1px solid #e2e8f0; }
    .text-right { text-align:right; }
    .summary { margin-top:30px; background: linear-gradient(135deg, #1e40af 0%, #2563eb 100%); color:white; padding:25px; border-radius:10px; text-align:center; }
      ...
      ${clientes
        .map((c, idx) => `
          <tr>
            ...
            <td class="text-right">${(c.rutas || []).filter(r => r.activa !== false).length}</td>
            <td class="text-right">${Number(c.totalViajes || 0)}</td>
            <td class="text-right"><strong>${Number(c.montoTotalGeneral || 0).toFixed(2)}</strong></td>
          </tr>
        `)
        .join("")}
    </tbody>
...
`;
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=viajes-consolidado-${anoNum}.pdf`
    );
    res.send(pdfBuffer);
  } catch (error) {
    if (browser) await browser.close();
