  ...
  const openEditModal = useCallback(async (truck) => {
    ...
    try {
      ...
      const [truckResponse, proveedoresResponse, motoristasResponse] = await Promise.all([
        fetch(`https://riveraproject-production-933e.up.railway.app/api/camiones/${truck.id}`, { credentials: 'include', ...fetchOptions }),
        fetch('https://riveraproject-production-933e.up.railway.app/api/proveedores', { credentials: 'include', ...fetchOptions }),
        fetch('https://riveraproject-production-933e.up.railway.app/api/motoristas', { credentials: 'include', ...fetchOptions })
      ]);
