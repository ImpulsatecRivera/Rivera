...
export const AuthProvider = ({ children }) => {
  ...
  const verifyTokenWithBackend = async (token) => {
    try {
      const response = await fetch('riveraproject-production.up.railway.app/api/login/checkAuth', {
        method: 'GET',
        headers: {
  ...
  const completeGoogleProfile = async (profileData) => {
    try {
      ...
      const response = await fetch('riveraproject-production.up.railway.app/api/login/complete-profile', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
