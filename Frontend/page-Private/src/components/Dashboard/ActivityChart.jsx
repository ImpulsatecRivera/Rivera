      ...
      const fetchStats = async () => {
        ...
        try {
          const res = await axios.get(
            `http://localhost:4000/api/viajes/estadisticas?tipo=${tipo}`
          );

          // Mapeamos los datos para adaptarlos al gráfico
          const mappedData = res.data.data.map((item) => {
            let label = "";
            if (typeof item._id === "object") {
              // Para el caso semana, que puede ser objeto {year, week} o similar
              if ("isoWeek" in item._id) {
                label = `Semana ${item._id.isoWeek}`;
              } else if ("year" in item._id && "week" in item._id) {
            ...
            return {
              label,
              viajes: item.totalViajes,
              completados: item.completados,
              progresoPromedio: item.progresoPromedio,
            };
          });
