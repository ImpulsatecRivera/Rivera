...
const SuccessLottieModal = ({ isOpen, onClose, title, message, buttonText = "Continuar" }) => {
  ...
  return (
    ...
    >
      ...
      {/* Modal */}
      <div className="relative z-10 bg-white rounded-2xl shadow-2xl p-8 mx-4 max-w-md w-full transform transition-all duration-300 scale-100">
...
// Hook personalizado para manejar el modal de éxito
export const useSuccessModal = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [config, setConfig] = useState({});

  const showSuccess = ({ title, message, onClose, buttonText }) => {
    setConfig({ title, message, onClose, buttonText });
    setIsOpen(true);
  };
...
export default SuccessLottieModal;