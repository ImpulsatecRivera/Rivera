...
const EmployeeDetailPanel = ({
  ...
  const safeText = (v, fallback = 'No especificado') => {
    const s = (v ?? '').toString().trim();
    return s ? s : fallback;
  };

  const formatSalary = (salario) => {
    if (salario === null || salario === undefined || salario === '') return 'No especificado';
    const num = Number(salario);
    if (Number.isNaN(num)) return 'No especificado';
      ...
      <div className="w-96 bg-white rounded-2xl shadow-2xl relative overflow-hidden flex flex-col h-full">
        ...
        >
          ...
          <div className="absolute inset-0 overflow-hidden">
            <div
              className="absolute top-10 left-10 w-20 h-20 rounded-full opacity-10 animate-pulse floating-animation"
              style={{ backgroundColor: '#5F8EAD' }}
            ></div>
            <div
              className="absolute bottom-10 right-10 w-16 h-16 rounded-full opacity-10 animate-pulse floating-animation-reverse"
              style={{ backgroundColor: '#5D9646' }}
            ></div>
            <div
              className="absolute top-1/2 left-4 w-12 h-12 rounded-full opacity-10 animate-pulse floating-animation-slow"
              style={{ backgroundColor: '#5F8EAD' }}
            ></div>
          </div>
          ...
          <div className="text-center z-10">
            ...
            <div className="flex justify-center space-x-3 mb-8">
              <div className="relative">
                <div
                  className="w-4 h-4 rounded-full bounce-custom"
            ...
            <div className="w-80 mx-auto">
              <div className="w-full bg-gray-600 rounded-full h-2 mb-4 overflow-hidden shadow-inner">
                <div
                  className="h-2 rounded-full relative overflow-hidden loading-wave"
                  style={{
                    background: 'linear-gradient(90deg, #5F8EAD 0%, #5D9646 50%, #5F8EAD 100%)',
                    width: '100%'
                  }}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white to-transparent opacity-40 shimmer-effect"></div>
                </div>
              </div>
        ...
        <style>{`
          .loading-wave { animation: loading-wave 2.5s ease-in-out infinite; }
          .bounce-custom { animation: bounce-custom 1.6s ease-in-out infinite both; }
          .floating-animation { animation: float 3s ease-in-out infinite; }
          .floating-animation-reverse { animation: float 3s ease-in-out infinite reverse; }
          .floating-animation-slow { animation: float 4s ease-in-out infinite; }
          .shimmer-effect { animation: shimmer 1.5s ease-in-out infinite; }
          .text-fade { animation: text-fade 3s ease-in-out infinite; }
  ...
  return (
    <div className="w-96 bg-white rounded-2xl shadow-2xl relative overflow-hidden flex flex-col h-full">
      ...
      <div
        ...
        style={{ backgroundColor: '#5F8EAD', borderRadius: '0 0 0 100%' }}
      ></div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="text-sm text-gray-600 break-words bg-white p-3 rounded-lg border">
                  {safeText(selectedEmpleados?.email)}
                </div>
              </div>
              <div>
                <div className="text-sm font-medium text-gray-700 mb-1">DUI</div>
                <div className="text-sm text-gray-600 bg-white p-3 rounded-lg border">
                  {safeText(selectedEmpleados?.dui)}
                </div>
              </div>

              {/* ✅ NUEVOS CAMPOS: Rol + Planilla Tipo */}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="text-sm font-medium text-gray-700 mb-1">Rol</div>
                  <div className="text-sm text-gray-600 bg-white p-3 rounded-lg border flex items-center">
                    <Shield className="w-4 h-4 mr-2" style={{ color: '#5F8EAD' }} />
                    {safeText(selectedEmpleados?.rol)}
                  </div>
                </div>
                <div>
                  <div className="text-sm font-medium text-gray-700 mb-1">Planilla</div>
                  <div className="text-sm text-gray-600 bg-white p-3 rounded-lg border flex items-center">
                    <ClipboardList className="w-4 h-4 mr-2" style={{ color: '#5F8EAD' }} />
                    {safeText(selectedEmpleados?.planillaTipo)}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="text-sm text-gray-600 bg-white p-3 rounded-lg border flex items-center">
                  ...
                  {formatDate(selectedEmpleados?.birthDate)}
                </div>
              </div>
              <div>
                ...
                <div className="text-sm text-gray-600 bg-white p-3 rounded-lg border flex items-center">
                  ...
                  {safeText(selectedEmpleados?.phone)}
                </div>
              </div>
              <div>
                ...
                <div className="text-sm text-gray-600 bg-white p-3 rounded-lg border flex items-center">
                  ...
                  {safeText(selectedEmpleados?.address)}
                </div>
              </div>
            </div>
          </div>
