      ...
      >
            ...
            {menuItems.map((item) => (
              <li key={item.name} className="relative">
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `menu-button w-full text-left rounded-lg relative overflow-hidden block transition-all duration-300 ${isActive ? 'active text-white opacity-100' : 'text-gray-300 opacity-75'
                    } ${isAnimating ? 'disabled' : 'cursor-pointer'} ${isTabletCollapsed ? 'px-2 py-2 text-center' : 'px-3 sm:px-4 py-2 sm:py-3'
                    }`
                  }
                  title={isTabletCollapsed ? item.name : ''}
                >
                  <span
                    className={`menu-text transition-opacity duration-300 ${animatingItem === item.name ? 'opacity-30' : 'opacity-100'
        ...
        <div className={`border-t transition-all duration-300 ${isTabletCollapsed ? 'p-2' : 'p-3 sm:p-4'}`} style={{ borderColor: '#4A4B50' }}>
          <button
            ...
            className={`logout-button w-full flex items-center text-gray-300 rounded-lg opacity-75 transition-all duration-300 ${isLogoutLoading ? 'loading' : 'cursor-pointer'
